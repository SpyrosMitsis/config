...
state=$(nmcli -t -f TYPE,state connection show --order name --active 2>/dev/null | grep tuni| head -1 | cut -d ':' -f 2)
timestamp=$(nmcli -t -f TYPE,TIMESTAMP connection show --order name --active 2>/dev/null | grep tun | head -1 | cut -d ':' -f 2)
vpn="$(nmcli -t -f NAME,TYPE connection show --order name --active 2>/dev/null | grep tun | head -1 | cut -d ':' -f 1)"
